...
Notice how the two `print` statements inside the `if` are indented by **4 spaces**. They belong to the `if` block. The last `print` is not indented, so it is **outside** the block.
